  ...
  app
    ...
    .message(/active/i, ['direct_mention', 'direct_message'], (msg) => {
      request({
          ...
          qs: {token: msg.meta.app_token, presence: true}, //Query string data
          ...
          if (error) {
            ...
            var active = _.without(_.map(j.members, function(o) {
              if(o.presence == 'active') return o.name + " (" + o.real_name + ")";
            }), undefined);
            msg.say(_.join(active,', '));
          }
      })
    })
    .message(/plus/i, ['direct_mention', 'direct_message'], (msg) => {
      request({
          url: 'https://slack.com/api/users.list', //URL to hit
          qs: {token: msg.meta.app_token, presence: true}, //Query string data
          method: 'GET', //Specify the method
          ...
          if (error) {
            ...
            var active = _.without(_.map(j.members, function(o) {
              if(o.presence == 'active') return o.profile.display_name + " (" + o.real_name + ")";
            }), undefined);
            msg.say(_.join(active,', '));
          }
      })
